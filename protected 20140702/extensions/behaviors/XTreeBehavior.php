...
class XTreeBehavior extends CActiveRecordBehavior
{
	...
	{
		$parents=array();
		if ($showRoot===false && $model->parent->{$this->topid} > 0)
		{
	...
	public function deleteKeepChildren()
	{
		...
		foreach($model->children as $child)
		{
			$child->{$this->topid}=$model->{$this->topid};
			$child->update($this->topid);
		}
		$model->delete();
	}
	...
	{
		$owner=$this->getOwner();
		$parentId=$owner->getAttribute($this->topid);
		$newParent=$owner->findbyPk($parentId);
		if($newParent===null)
		{
			$owner->addError($this->topid, Yii::t('treeBehavior','Parent node does not exist.'));
			$event->isValid=false;
		}
		...
		if (!$owner->getIsNewRecord())
		{
			...
			if($parentId==$id)
			{
				$owner->addError($this->topid,Yii::t('treeBehavior','This parent node is not allowed, because node cannot be child of itself.'));
				$event->isValid=false;
			}
			$oldModel=$owner->findbyPk($id);
			if($oldModel->topid!=$parentId)
			{
				foreach($this->getParents($newParent) as $parent)
				{
					if($parent->tid==$id)
					{
						$owner->addError($this->topid, Yii::t('treeBehavior','This parent node is not allowed, because it is child node of the given node.'));
						$event->isValid=false;
						break;
					}
				}
